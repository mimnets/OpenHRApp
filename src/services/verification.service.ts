...
export const verificationService = {
  ...
  async waitForEmailVerification(
    ...
    return new Promise((resolve) => {
      const checkInterval = setInterval(async () => {
        ...
        try {
          // Try to login - if verified, it will succeed
          // This is just to check verification status
          await apiClient.pb?.collection('users').getOne('');
          ...
          // Expected - user not verified yet
        }
  ...
  /**
   * Verify user email with token (called from email link)
   * Uses PocketBase's built-in confirmVerification method
   */
  async verifyEmailToken(token: string): Promise<{ success: boolean; email?: string; message: string }> {
    ...
    try {
      ...
      await apiClient.pb.collection('users').confirmVerification(token);
      ...
      if (err.response?.message) {
        ...
        if (err.message.includes('expired') || err.message.includes('invalid')) {
          errorMsg = "Verification link has expired or is invalid. Please request a new one.";
