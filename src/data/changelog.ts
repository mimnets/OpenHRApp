...
export const changelog: ChangelogRelease[] = [
  {
    ...
    entries: [
      { type: 'fix', description: 'Super Admin "Delete Organization" was failing with a 400 `Make sure that the record is not part of a required relation reference` after partially wiping users/teams/leaves/attendance/settings, leaving orphan org rows behind. `superAdminService.deleteOrganization()` now sweeps every child collection that carries `organization_id` — `attendance`, `leaves`, `performance_reviews`, `review_cycles`, `notifications`, `announcements`, `reports_queue`, `upgrade_requests`, `shifts`, `teams`, `settings`, `users` — in dependency order (timesheets/leaves/reviews before users/teams) before deleting the org itself. Per-record deletes also tolerate 404s (already cascaded) and log other failures instead of aborting, so one stuck row no longer blocks the rest of the cleanup' },
